...
def test_lorenz(device='cpu', model_file_saved=None, test_data_file=None, test_logfile=None, evaluation_mode='Full', p=0.5, bias=30):
    ...
    if "partial" in evaluation_mode:
        ...
        sigma_e2_dB_test = partial_corrupt((sigma_e2_dB_test), p=p, bias=bias)
...
if __name__ == "__main__":
    ...
    for smnr_dB in smnr_dB_arr:
        ...
        #model_file_saved_dict["{}dB".format(smnr_dB)] = glob.glob("./models/*LorenzSSM_danse_opt_*_m_3_n_3_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]
    ...
    test_logfile = "./log/Lorenz_test_{}_T_{}_N_{}_log.log".format(evaluation_mode, T_test, N_test)
    test_jsonfile = "./log/Lorenz_test_{}_T_{}_N_{}_results.json".format(evaluation_mode, T_test, N_test)
    ...
    for i, smnr_dB in enumerate(smnr_dB_arr):
        ...
        print(model_file_saved_i.replace('\\','/'))
    ...
    plt.legend()
    plt.tight_layout()
    tikzplotlib.save('./figs/LorenzModel/{}/InferTime_vs_SMNR_Lorenz.tex'.format(evaluation_mode))
    plt.savefig('./figs/LorenzModel/{}/InferTime_vs_SMNR_Lorenz.pdf'.format(evaluation_mode))
    ...
    plt.legend()
    plt.tight_layout()
    tikzplotlib.save('./figs/LorenzModel/{}/MSE_vs_SMNR_Lorenz.tex'.format(evaluation_mode))
    plt.savefig('./figs/LorenzModel/{}/MSE_vs_SMNR_Lorenz.pdf'.format(evaluation_mode))
