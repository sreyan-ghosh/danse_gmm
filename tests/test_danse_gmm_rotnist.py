...
def test_danse_rotnist(danse_model, saved_model_file, Y, Cw, device=None):
    ...
    with torch.no_grad():
        ...
        beta_estimated_filtered, Z_estimated_filtered, Pk_estimated_filtered = danse_model.compute_predictions(Y_test_batch, Cw_test_batch)
    
    return beta_estimated_pred, Z_estimated_pred, Pk_estimated_pred, \
        beta_estimated_filtered, Z_estimated_filtered, Pk_estimated_filtered
...
def test_rotnist(device=None, model_file_saved=None, test_data_file=None, test_logfile=None, evaluation_mode='Full', p=0.5, bias=30):
    ...
    dataset_type, rnn_type, m, n, n_mix, T, _, smnr_dB = parse("{}_danse_opt_{}_m_{:d}_n_{:d}_n_mix_{:d}_T_{:d}_N_{:d}_smnr_{:f}dB", model_file_saved.split('/')[-2])
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--n_mix", help="Enter number of mixtures", type=int)
    ...
    for smnr_dB in smnr_dB_arr:
        model_file_saved_dict["{}dB".format(smnr_dB)] = glob.glob(r"./models/*rotnist_danse_opt_*n_16_n_mix_{}_T_{}_N_{}_smnr_{}dB*/*best*".format(n_mix, T_train, N_train, smnr_dB))[-1]
